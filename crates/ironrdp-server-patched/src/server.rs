...
pub enum ServerEvent {
    ...
    /// Send pre-encoded data through a DVC channel.
    /// `channel_id` is the DVC channel ID (from DvcProcessor::start),
    /// `data` is the encoded PDU payload to send on that channel.
    Dvc {
        channel_id: u32,
        data: Vec<u8>,
    },
...
impl RdpServer {
    ...
    /// Set a builder function to add custom DVC channels (e.g., GFX for H.264).
    /// The function receives the DrdynvcServer and should return it with channels added.
    pub fn set_dvc_builder(
        &mut self,
    ...
    async fn dispatch_pdu(
        ...
        trace!(?action, bytes_len = bytes.len(), "dispatch_pdu: received PDU");
        match action {
            Action::FastPath => {
    ...
    async fn dispatch_server_events(
        ...
        for event in events.drain(..) {
            ...
            match event {
                ...
                ServerEvent::Dvc { channel_id, data } => {
                    let Some(drdynvc_svc_id) = self.get_channel_id_by_type::<dvc::DrdynvcServer>() else {
                        warn!("No DRDYNVC channel, dropping DVC event");
                        continue;
                    };
                    // Split data into DVC-sized chunks and send as SVC messages
                    let total_length = data.len();
                    let max_chunk = dvc::pdu::DrdynvcDataPdu::MAX_DATA_SIZE;
                    let needs_splitting = total_length > max_chunk;
                    let mut off = 0;
                    while off < total_length {
                        let remaining = total_length - off;
                        let size = remaining.min(max_chunk);
                        let chunk = data[off..off + size].to_vec();
                        let dvc_pdu = if needs_splitting && off == 0 {
                            dvc::pdu::DrdynvcServerPdu::Data(dvc::pdu::DrdynvcDataPdu::DataFirst(
                                dvc::pdu::DataFirstPdu::new(channel_id, total_length as u32, chunk),
                            ))
                        } else {
                            dvc::pdu::DrdynvcServerPdu::Data(dvc::pdu::DrdynvcDataPdu::Data(
                                dvc::pdu::DataPdu::new(channel_id, chunk),
                            ))
                        };
                        let svc_msg = ironrdp_svc::SvcMessage::from(dvc_pdu)
                            .with_flags(ironrdp_svc::ChannelFlags::SHOW_PROTOCOL);
                        let encoded = server_encode_svc_messages(
                            vec![svc_msg],
                            drdynvc_svc_id,
                            user_channel_id,
                        )?;
        ...
        W: FramedWrite,
    {
        ...
        let dispatch_events = async move {
            ...
            let mut event_batch_count: u64 = 0;
            loop {
                ...
                event_batch_count += 1;
                if event_batch_count % 60 == 1 {
                    debug!(batch = event_batch_count, count = events.len(), "dispatch_events: processing batch");
                }
                let mut this = this.lock().await;
                match this
        ...
        W: FramedWrite,
    {
            ...
            .await
        {
            Ok(state) => state,
            Err(e) => {
                error!("client loop failed: {e:#}");
                return Err(e.context("client loop failure"));
            }
        };
    ...
    async fn handle_fastpath(&mut self, input: FastPathInput) {
        trace!("handle_fastpath: {} input events", input.input_events().len());
